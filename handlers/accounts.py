...
async def cb_add_account(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("Введите любое имя сессии (например: malot1):")
...
async def process_session_name(message: Message, state: FSMContext):
    ...
    await message.answer(f"⏳ Запускаю авторизацию для <b>{session_name}</b>...\n\nPyrogram попросит номер телефона → код из SMS → пароль (если есть).", parse_mode="HTML")
    ...
    try:
        ...
        await message.answer(
            ...
            "Теперь можно использовать кнопку «🚀 Новая рассылка»",
