# bot.py
# ФИНАЛЬНАЯ СТАБИЛЬНАЯ ВЕРСИЯ 2026
# Полностью внутри Telegram + автоматическое лечение AuthKeyUnregisteredError
...
from telethon.errors import FloodWaitError, PeerFloodError, SessionPasswordNeededError, AuthKeyUnregisteredError
...
async def show_main_menu(event):
    ...
    await event.respond("Главное меню рассылки:", buttons=buttons)
...
async def register_handlers():
    ...
    async def auth_handler(event):
        ...
        if step == 'phone':
            ...
            try:
                ...
                await event.respond("✅ 2FA пройден!")
                del temp_data[uid]
                load_settings()
                await show_main_menu(event)
            except Exception as e:
    ...
    async def choose_chats(event):
        ...
        for i, d in enumerate(dialogs, 1):
            ...
            text += f"{i}. {name}\n"
        await event.respond(text + "\nОтправьте номера через запятую или «все»")
    ...
    async def save_photo(event):
        ...
        try:
            ...
            await event.respond(f"Ошибка фото: {e}")
        del temp_data[uid]
        await show_main_menu(event)
    ...
    async def help_cmd(event):
        if not await is_owner(event): return
        await event.respond("Бот работает полностью внутри Telegram.\n/start — меню\nстоп — остановить рассылку")

# ========================= ЗАПУСК С АВТО-ЛЕЧЕНИЕМ СЕССИИ =========================
async def main():
    global client
    ...
    while True:
        try:
            ...
            if await client.is_user_authorized():
                logger.info("Аккаунт авторизован успешно")
            else:
                logger.info("Требуется авторизация — отправьте /start")

            load_settings()
            await register_handlers()
            logger.info("Бот полностью запущен и готов к работе внутри Telegram.")
            await client.run_until_disconnected()
            break

        except AuthKeyUnregisteredError:
            logger.error("AuthKeyUnregisteredError — сессия повреждена. Автоматически удаляем файл...")
            if os.path.exists(session_file):
                os.remove(session_file)
            if client:
                await client.disconnect()
            logger.info("Сессия очищена. Напишите /start для новой авторизации.")
            await asyncio.sleep(3)
